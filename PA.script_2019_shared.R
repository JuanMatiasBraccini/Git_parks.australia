...
if(User=="Sarah") setwd("~/GitHub/shark_analysis")
 # Clean data
source("underwater_clean_loop.R") #takes 2.5 minutes to load


#3. Species list
if(!exists('All.species.names'))
{
...
setwd(paste0(hndl.Sarah,'/Data'))
if(User=="Abbey") setwd("C:/Users/A.Shutt/OneDrive - Department of Primary Industries And Regional Development/Masters_Rscript_2023")
if(!exists('TEPS')) TEPS <- read_excel("TEPS interactions.xlsx", sheet = "Sheet1",skip = 1)


#5. PA - number of hook combinations used and lost in PA project
if(!exists('Hook.combos')) Hook.combos <- read_excel("Hook count.xlsx", sheet = "Sheet1",skip = 0)
if(!exists('Lost.snoods')) Lost.snoods<- read_excel("Broken hook specs.xlsx",sheet = "Sheet1")

#6. PA - battery life
setwd(hndl.Sarah)
D1.cam.battery <- read.csv("Metadata Checks/Deck 1 final metadata check.csv") %>% 
  dplyr::select(x, Full.LL, Full.GN, More.comments) %>% 
  rename(`DIPRD code` = x)
D2.cam.battery <- read.csv("Metadata Checks/Deck 2 final metadata check.csv")%>% 
  dplyr::select(x, Full.LL, Full.GN, More.comments) %>% 
  rename(DPIRD.code = x)
...
#--------- Redundant analyses ------------
already.in_PA.script_2019=TRUE
if(!already.in_PA.script_2019)
{
  Do.tiff="YES"
  Do.jpeg="NO"
